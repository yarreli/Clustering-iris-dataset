...
# Recovering the original variety, and assigning an numerical value
variety_dict ={'Versicolor' : 0, 'Setosa' : 1, 'Virginica' : 2} # Create a dictionary

vari = pd.DataFrame({'variety': iris.variety})
vari['variety_num'] = vari['variety'].apply(set_value, args =(variety_dict, )) 

# only numerical values 
xx = iris.drop('variety', axis=1)
## ============ 

# fit a list of models to choose 3, according to the accuracy
import sklearn.metrics as sm
#  If linkage is “ward”, only “euclidean” is accepted.
Hclus = AgglomerativeClustering(n_clusters=3, affinity="euclidean", linkage='ward')
Hclus.fit(xx)
print ("Accuracy for a model with affinity=euclidean and linkage= ward:")
...
dat_en = {'vari': vari, 'x':iris.drop('variety', axis=1)}
